...
@Composable
fun CreateGoldTaxScreen(
	vm: CreateGoldTaxVM = viewModel(factory = CreateGoldTaxVM.Factory),
) {
	...
	LaunchedEffect(Unit) {
		vm.effect.collect { effect ->
			when (effect) {
				is CreateGoldTaxEffect.ShowMessage -> snackbarHostState.showSnackbar(effect.message)
				CreateGoldTaxEffect.NavigateBack -> backStack.removeLastOrNull()
			}
		}
	}
...
@Composable
private fun PreviewCreateGoldTaxScreen() {
	ArtaTheme {
		Content(
